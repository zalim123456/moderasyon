...
if(kanal){
if(member.user.bot){
  if(member.id == izinli) return client.channels.cache.get(kanal).send(`${member.user.username} Adlı Bot Sunucuya Katıldı ama İzinli Olduğu İçin Girmesine İzin Verdim`)
member.members.ban().then(client.channels.cache.get(kanal).send(`${member.user.username} Adlı Bot Sunucuya Katıldı ve Onu Banladım`))
}else return;
}else return;
})
...
if (!cezalımı) return;
if (cezalımı == "cezali") {
member.roles.add(`803894592025133086`)
...
db.delete(`cezali_${member.guild.id + member.id}`)
    ...
    member.roles.remove('803894592025133086');
  }, ms(sürejail));
}
})
...
if (!mutelimi) return;
if (mutelimi == "muteli") {
member.roles.add(`803894592717586472`)
...
db.delete(`muteli_${member.guild.id + member.id}`)
    ...
    member.roles.remove('803894592717586472');
  }, ms(süre));
}
})
...
client.on("ready", () => {
  client.channels.cache.get("803909001497673738").join();
})
//--------------------------------------------------------------------------//
